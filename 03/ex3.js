...
// only one character (for example, "b"). The first character
// has position zero, which causes the last one to be found at
// position string.length - 1. In other words, a two-character
...
const countChar = (word, letter) => {
  ...
  for(let i = 0; i < (word.length); i++) {
    if(word.charAt(i) === letter) occurances++;
  }
...
// Book solutions:
function bookCountChar(string, ch) {
  var counted = 0;
